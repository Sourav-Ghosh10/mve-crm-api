...
async function seedPast6MonthsData() {
    try {
        ...
        for (const user of users) {
            console.log(`Adding past 6 months attendance for user: ${user.firstName} ${user.lastName}`);
            
            // Loop through each day in the past 180 days
            for (let d = new Date(sixMonthsAgo); d <= today; d.setDate(d.getDate() + 1)) {
                ...
                const attendance = new Attendance({
                    ...
                    checkIn: {
                        time: checkInTime,
                        ipAddress: '127.0.0.1',
                        deviceInfo: 'Past 6 Months Dummy Script',
                    },
                    checkOut: {
                        time: checkOutTime,
                        ipAddress: '127.0.0.1',
                    ...
                    sessions: [{
                        checkIn: { time: checkInTime, ipAddress: '127.0.0.1', deviceInfo: 'Past 6 Months Dummy Script' },
                        checkOut: { time: checkOutTime, ipAddress: '127.0.0.1', deviceInfo: 'Past 6 Months Dummy Script' },
                        isLate,
                        isEarlyLeave
                    }],
                    status: 'present',
