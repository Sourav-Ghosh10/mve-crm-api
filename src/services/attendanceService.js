...
const attendanceService = {
    ...
    getAttendanceStatus: async (userId) => {
        ...
        const [attendance, holidays, schedules, monthlyAttendances] = await Promise.all([
            ...
            }).select('overtime').lean()
        ]);

        const monthlyAttendanceCount = monthlyAttendances.length;
        const monthlyOvertimeHours = monthlyAttendances.reduce((sum, record) => sum + (record.overtime || 0), 0);
        const otHours = Math.floor(monthlyOvertimeHours);
        const otMins = Math.round((monthlyOvertimeHours - otHours) * 60);
        const monthlyOvertimeString = `${otHours}h ${otMins}m`;
